...
const getDepartamentos = async () => {
    try{
        const request = await axios.get(`${url}/departamento/`, tokenService.GetTokenPrueba());
        return request.data;
    } catch(except){
...
const registerDepartamento = async newObject => {
    try {
        ...
        const request = await axios.post(`${url}/departamento/`, newObject, tokenService.GetTokenPrueba());
        return request.data;
    } catch(except) {
        console.error(except)
    }
